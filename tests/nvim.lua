...
local server
local function wait(predicate) assert(vim.wait(10000, predicate, 5), 'asynchronous operation did not finish') end
local ok, failure = xpcall(function()
  local project = require('pdfterm.project')
  local terminal = require('pdfterm.terminal')
  terminal.capture_source = function() error('attachment must not require terminal capture') end
  local log = directory .. '/build.log'
  local completed = {}
  vim.fn.mkdir(directory .. '/other')
  local function build(tag, id)
    project.build({ cwd = tag == 'A' and directory or directory .. '/other', pdf = directory .. '/shared.pdf',
      build = { '/bin/sh', '-c', 'echo start:$1 >> "$2"; sleep .05; echo end:$1 >> "$2"', 'test', tag, log } }, id,
      function(result) assert(result.code == 0, result.stderr); completed[#completed + 1] = tag end)
  end
  build('A', project.next())
  build('B', project.next())
  build('C', project.next())
  wait(function() return #completed == 1 end)
  assert(completed[1] == 'C', 'obsolete build completion navigated')
  assert(table.concat(vim.fn.readfile(log), ',') == 'start:A,end:A,start:C,end:C', 'builds overlapped or obsolete pending build ran')
  local result
  project.run({ '/bin/sh', '-c', 'sleep 30 & wait' }, directory, 40, function(value) result = value end)
  wait(function() return result ~= nil end)
  assert(result.code ~= 0 and result.stderr:find('timed out'))
  result = nil
  project.run({ '/bin/sh', '-c', 'yes flood' }, directory, 1000, function(value) result = value end)
  wait(function() return result ~= nil end)
  assert(result.code ~= 0 and result.stderr:find('1 MiB'))

  vim.fn.writefile({ 'forward_socket="forward.sock"', '[editor]', 'transport="socket"', 'path="editor.sock"' }, directory .. '/pdfterm/config.toml')
  ...
  vim.fn.writefile({ '#!/bin/sh', 'echo selected >> ' .. vim.fn.shellescape(directory .. '/bootstrap.log'), 'exec ' .. vim.fn.shellescape(binary) .. ' "$@"' }, wrapper)
  ...
  adapter.setup({ executable = wrapper, session = 'adapter', attach_only = true, compile = true,
    project = { main = 'navigation.tex', pdf = 'artifacts/navigation.pdf', cwd = directory,
      build = { '/bin/sh', '-c', 'sleep .05; exec pdflatex -interaction=nonstopmode -halt-on-error -synctex=1 -output-directory=artifacts navigation.tex' } } })
  ...
  local config = vim.json.decode(vim.system({ binary, '--session', 'adapter', '--print-config' }, { text = true }):wait(10000).stdout)
  ...
  server:listen(16, function(error)
    ...
    local client = assert(vim.uv.new_pipe(false)); server:accept(client)
    local chunks = {}
    client:read_start(function(read_error, chunk)
      ...
      if chunk then chunks[#chunks + 1] = chunk
        ...
        client:write('{"ok":true,"error":null}', function() client:shutdown(function() client:close() end) end)
      end
    end)
  end)
  vim.cmd.edit(vim.fn.fnameescape(source))
  vim.api.nvim_win_set_cursor(0, { 4, 0 }); adapter.forward()
  vim.api.nvim_win_set_cursor(0, { 6, 0 }); adapter.forward()
  local ticks = 0
  local timer = assert(vim.uv.new_timer()); timer:start(1, 5, function() ticks = ticks + 1 end)
  wait(function() return #requests == 1 end)
  timer:stop(); timer:close()
  assert(ticks > 2, 'build/resolution blocked Neovim events')
  local resolved = vim.system({ binary, pdf, '--synctex-view', source, '--line', '6', '--column', '1' }, { text = true }):wait(10000)
  ...
  inverse:connect(config.editor.path, function(error)
    ...
    inverse:write(vim.json.encode({ file = source, line = 4, byte_column = 6 }), function()
      inverse:shutdown(function() inverse:close() end)
    end)
  end)
  wait(function() return vim.api.nvim_win_get_cursor(0)[1] == 4 end)
  assert(vim.api.nvim_win_get_cursor(0)[2] == 6)
  print('adapter regressions passed: serialized/latest build, timeout/output cap, bootstrap, named session, attach-only, inverse socket; event ticks=' .. ticks)
end, debug.traceback)
...
vim.api.nvim_create_autocmd('VimLeavePre', { once = true, callback = function() vim.fn.delete(directory, 'rf') end })
